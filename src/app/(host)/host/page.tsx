...
export default async function HostDashboardPage() {
  const [profile, statsResult, bookingsResult, propertiesResult, reportsResult] =
    await Promise.all([
      getCurrentProfile(),
      getDashboardStatsAction(),
      listBookingsAction(),
  ...
  const report: DashboardReport | null = reportsResult.ok
    ? reportsResult.data
    : null;
  ...
  return (
    <div className="p-4 sm:p-6 lg:p-8 max-w-7xl mx-auto">
      ...
      {/* ── Charts row: Revenue + Booking status ── */}
      ...
      {report && (
        <div className="mt-6 grid gap-4 sm:grid-cols-3">
          ...
          <MiniMetric
            label="Tổng đặt cọc"
            value={formatVND(report.totalDeposit)}
            icon="deposit"
          />
          <MiniMetric
            label="Tỷ lệ Occupancy"
                  ...
                  <div className="flex items-center gap-3 min-w-0">
                    ...
                    <div className="min-w-0">
                      ...
                      <p className="text-xs text-ink-500">
                        {r.bookings} booking ·{' '}
                        {r.occupancy.toFixed(0)}% occupancy
                      </p>
                    </div>
                  </div>
