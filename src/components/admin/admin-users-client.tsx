...
const ROLE_VARIANT: Record<RoleCode, Parameters<typeof Badge>[0]['variant']> = {
  [RoleCode.ADMIN]: 'dark',
  [RoleCode.OWNER]: 'navy',
  [RoleCode.SALE]: 'info',
...
/**
 * Người dùng fetch từ `/api/admin/users` PHÍA CLIENT → endpoint hiện trong F12
 * Network. Filter role/status/q (URL) + phân trang in-memory.
 */
export function AdminUsersClient({ role, status, q }: Props) {
  ...
  return (
    <tr className="transition-colors hover:bg-cream-50">
      <td className="px-5 py-4">
        <div className="flex items-center gap-3">
          ...
          <div className="min-w-0">
            ...
            <p className="text-[11px] font-mono text-ink-400 truncate">{u.id}</p>
          </div>
        </div>
      </td>
      <td className="px-5 py-4">
      ...
      <td className="px-5 py-4 text-right text-xs text-ink-700">
        ...
        {u.disputeCount > 0 && (
          <p className="font-medium text-rose-600">{u.disputeCount} khiếu nại</p>
        )}
      </td>
      <td className="px-5 py-4">
