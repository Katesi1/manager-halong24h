...
const STATUS_VARIANT: Record<BankStatus, Parameters<typeof Badge>[0]['variant']> =
  {
    none: 'default',
    pending: 'gold',
    approved: 'success',
...
/**
 * Hàng đợi duyệt STK fetch từ `/api/admin/bank-accounts` PHÍA CLIENT → endpoint
 * hiện trong F12 Network. Phân trang + filter server-side.
 */
export function AdminBankAccountsClient({
  ...
  const { loading, error, data } = useApiResource<BankAccountQueueResult>(
              ...
              {t.key === 'pending' && pendingCount > 0 && (
                <span
                  className={
                    ...
                    (active ? 'bg-white text-navy-900' : 'bg-rose-100 text-rose-700')
                  }
                >
                  {pendingCount}
                </span>
