...
/**
 * Gói cước fetch từ `/api/host/subscription` PHÍA CLIENT → endpoint hiện trong
 * F12 Network.
 */
export function SubscriptionClient() {
  const { loading, error, data } = useApiResource<SubData>(
    ...
    <div className="p-4 sm:p-6 lg:p-8 max-w-3xl mx-auto">
      ...
      {!loading && (error || !data) && (
        <div className="rounded-lg bg-rose-50 px-4 py-3 text-sm text-rose-900 ring-1 ring-rose-200">
          {error ?? 'Không tải được gói cước'}
        </div>
      )}

      {!loading && data && <SubBody sub={data.sub} blocked={data.blocked} />}
    </div>
  );
}
    ...
    <>
      ...
      <section className="rounded-2xl bg-white p-6 ring-1 ring-ink-200/60 shadow-card">
        ...
        <dl className="mt-5 grid gap-3 sm:grid-cols-3">
          ...
          />
          {sub.paidAt && <Detail label="Đã thu" value={formatDate(sub.paidAt)} />}
        ...
        {(sub.status === 'past_due' || sub.status === 'trial') && (
          <div className="mt-5 rounded-lg bg-amber-50 p-4 ring-1 ring-amber-200">
            ...
            <p className="mt-2 text-[11px] text-amber-700">
              Sau khi chuyển, gọi 0900-xxxxx hoặc chat để admin ghi nhận. Trong
              tương lai sẽ tự động xác nhận qua webhook bank.
            </p>
          </div>
        )}
