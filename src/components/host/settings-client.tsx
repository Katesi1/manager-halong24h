...
/**
 * Cài đặt cá nhân fetch từ `/api/host/settings` PHÍA CLIENT → endpoint hiện
 * trong F12 Network. Các form con (Profile/Password/Notifications) tự submit.
 */
export function SettingsClient() {
  const { loading, error, data } = useApiResource<SettingsData>(
    '/api/host/settings',
  );

  if (loading) {
    return (
      <div className="p-4 sm:p-6 lg:p-8 max-w-4xl mx-auto text-center text-sm text-ink-500">
        Đang tải…
      </div>
    );
  }
      ...
      <div className="mt-6 grid gap-6">
        ...
        {isOwner && kyc && (
          ...
          >
            ...
            {kyc.kycStatus !== 'approved' && !kyc.kycBypass && (
              <div className="mt-4">
                <Link
                  href="/host/kyc"
            ...
            <div className="flex flex-wrap items-center justify-between gap-3">
              <div className="flex items-center gap-2">
                ...
                <Badge variant={bankStatusVariant(profile.bankStatus)}>
                  {BANK_STATUS_LABEL[profile.bankStatus ?? 'none']}
                </Badge>
              </div>
              <Link
  ...
  return (
    ...
    >
      <div className="border-b border-ink-100 px-6 py-4 flex items-center gap-3">
        <div className="rounded-lg bg-cream-100 p-2 text-navy-700">{icon}</div>
