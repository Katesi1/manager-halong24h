...
/**
 * Trang gói cước fetch từ `/api/host/billing` PHÍA CLIENT → endpoint hiện trong
 * F12 Network. Hero + TierSelector + lịch sử hoá đơn.
 */
export function HostBillingClient() {
  const { loading, error, data } = useApiResource<BillingData>(
    '/api/host/billing',
  );
  ...
  const { roomCount, sub, plans, plansError, invoices, invoicesError } = data;
  const currentPlanId = sub?.planId || null;
  const currentLabel = currentPlanId ? planLabel(currentPlanId) : 'Miễn phí';
  const periodCost = sub && sub.amount > 0 ? formatVND(sub.amount) : 'Miễn phí';
    ...
    <>
      <section className="rounded-2xl overflow-hidden shadow-lg">
        ...
        <div className="grid grid-cols-3 divide-x divide-ink-200 bg-white">
          ...
          <QuickStat
            label="Kỳ thanh toán"
            value={sub ? (sub.cycle === 'yearly' ? 'Theo năm' : 'Theo tháng') : '—'}
          />
          <QuickStat
            label="Trạng thái"
            value={
          ...
          <div>
            ...
            <ol className="mt-3 space-y-2 text-sm text-ink-700">
              <li className="flex items-start gap-2">
                <span className="grid h-5 w-5 shrink-0 place-items-center rounded-full bg-navy-900 text-[10px] font-bold text-white mt-0.5">
                  1
                </span>
            ...
            <p className="mt-3 text-xs text-ink-500">
              Quá hạn 7 ngày → cảnh báo. Quá 14 ngày → tạm khóa cơ sở cho tới khi
              thanh toán.
            </p>
          </div>
        </div>
          ...
          {invoices.map((inv) => (
            <tr key={inv.id} className="text-ink-700">
              ...
              <td className="px-4 py-3">
                ...
                {inv.cycle === 'yearly' ? (
                  <span className="block text-[11px] text-ink-500">Theo năm</span>
                ) : (
...
function QuickStat({
  ...
  return (
    <div className="px-5 py-3.5 text-center">
      ...
      <p
        className={`mt-0.5 font-display text-lg font-semibold ${valueClass ?? 'text-navy-900'}`}
      >
        {value}
      </p>
    </div>
